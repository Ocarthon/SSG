...
public class CuraConfiguration {
    ...
    public static Cura.SettingList readSettingsFromObject(Object obj) {
        ...
        for (Field field : obj.getClass().getDeclaredFields()) {
            if (!field.isAnnotationPresent(CuraSetting.class)) {
                continue;
            }

            CuraSetting curaSetting = field.getAnnotationsByType(CuraSetting.class)[0];
            try {
                System.out.println("SETTING " + curaSetting.key());
                settingList.addSettings(createSetting(curaSetting.key(), field.get(obj).toString()));
